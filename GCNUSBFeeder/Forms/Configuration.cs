...
namespace GCNUSBFeeder
{
    public partial class Configuration : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!port1Enabled.Checked) { JoystickHelper.DestroyJoystick(1); }
                else                       { JoystickHelper.CreateJoystick(1);  }
            }
            if ((bool)Settings.Default["port2Enabled"] != port2Enabled.Checked)
            {
                if (!port2Enabled.Checked) { JoystickHelper.DestroyJoystick(2); }
                else                       { JoystickHelper.CreateJoystick(2);  }
            }
            if ((bool)Settings.Default["port3Enabled"] != port3Enabled.Checked)
            {
                if (!port3Enabled.Checked) { JoystickHelper.DestroyJoystick(3); }
                else                       { JoystickHelper.CreateJoystick(3);  }
            }
            if ((bool)Settings.Default["port4Enabled"] != port4Enabled.Checked)
            {
                if (!port4Enabled.Checked) { JoystickHelper.DestroyJoystick(4); }
                else                       { JoystickHelper.CreateJoystick(4);  }
            }
